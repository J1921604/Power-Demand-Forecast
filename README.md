...
│   ├── data/
│   │   ├── data.py               # データ処理
│   │   ├── juyo-YYYY.csv         # 電力需要データ (2016-2025)
...
1. `AI/tomorrow/data.py` と `AI/tomorrow/temp.py` を同じ時刻帯で必ず連続実行し、電力実績と気温特徴量をそろえる

   ```powershell
   cd AI
   $env:PYTHONIOENCODING='utf-8'
   python tomorrow\data.py
   python tomorrow\temp.py
   ```

2. `tomorrow/Ytest.csv` の行数が **168**（7日 × 24時間）であることを確認

   ```powershell
   (Import-Csv AI/tomorrow/Ytest.csv).Count
   ```

3. その後 `python tomorrow\LightGBM\LightGBM_tomorrow.py` を再実行すると、R² が 0.89 付近まで戻ることを確認
4. GitHub Actions の場合もワークフロー内で「Fetch latest load data」→「Validate load window length」→「Fetch latest temperature data」の順序になっているか確認し、`Validate load window length` ステップのログに `Ytest rows: 168` が出力されていることを必ず確認
