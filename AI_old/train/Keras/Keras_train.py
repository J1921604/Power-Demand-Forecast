...
def load_training_data(xtrain_csv: str, 
    ...
    # ログ整合性確認用（server.logとの比較用）
    total_rows = X_train.shape[0] + X_test.shape[0]
    print(f"特徴量データ作成完了(合計): ({total_rows}, {X_train.shape[1]})")
...
def evaluate_model_performance(model: Sequential,
    ...
    # R2スコア精度閾値テスト (US1: 品質基準 >= 0.80)
    MIN_R2_SCORE = 0.80
    if r2 < MIN_R2_SCORE:
        warning_msg = f"[WARNING] R2スコア ({r2:.4f}) が閾値 ({MIN_R2_SCORE}) を下回っています"
        print(warning_msg)
        print("  -> データ品質またはハイパーパラメータの見直しを推奨")
    else:
        print(f"[OK] R2スコア閾値テスト合格: {r2:.4f} >= {MIN_R2_SCORE}")
