...
def download_and_extract_latest_data(
    ...
    """
    # 出力先ディレクトリ作成
    target_dir = os.path.dirname(juyo_target_path)
...
def create_tomorrow_prediction_dataset(
    ...
    Args:
        combined_df: マージ済みデータフレーム（前月・当月を統合済み）
        Ytest_csv: 出力CSVファイルパス
        past_days: 過去データ取得日数
        forecast_days: 予測日数
...
def process_temperature_and_create_dataset(
    ...
    try:
        # 出力先ディレクトリ作成
        output_dir = os.path.dirname(Ytest_csv)
        if output_dir:
            os.makedirs(output_dir, exist_ok=True)
        
        # 電力データの最新日時を取得
        # df_kwのインデックスはdatetime型であることを前提
        if len(df_kw) == 0:
            raise ValueError("電力データが空です")
        ...
        # 過去指定日数分のデータを抽出
        # 最新データから past_days 日前を暫定開始として範囲抽出し、
        # その後に target_hours (= past_days*24) ちょうどになるよう末尾を採用する。
        # NOTE: inclusive な範囲抽出 + tail の組み合わせだと開始がズレる可能性があるため、
        #       period_info.json には補正後の実開始/実終了を保存する。
        start_datetime = latest_data_datetime - dt.timedelta(days=past_days_int)
        print(f"開始日時(暫定): {start_datetime}")
        
        # 時刻を含めた範囲でデータ抽出
        df_kw = df_kw[(df_kw.index >= start_datetime) & (df_kw.index <= latest_data_datetime)]
        df_kw = df_kw.sort_index()

        # 取得データが過不足なくpast_days*24件になるように末尾を使用
        target_hours = past_days_int * 24
        if len(df_kw) > target_hours:
            df_kw = df_kw.tail(target_hours)
        elif len(df_kw) < target_hours:
            raise ValueError(f"必要なデータ行数({target_hours})を確保できませんでした: {len(df_kw)}行")
        print(f"抽出データ行数: {len(df_kw)}")
...
def main() -> None:
    ...
    try:
        ...
        # ファイルパス設定（絶対パス解決）
        # NOTE:
        # 予測系スクリプト（例: tomorrow/LightGBM/LightGBM_tomorrow.py）は
        #   - tomorrow/Ytest.csv
        #   - tomorrow/period_info.json
        #   - tomorrow/tomorrow.csv
        # を参照するため、生成物は `AI/tomorrow/` 直下に統一する。
        # ここで `AI/tomorrow/tomorrow/` に出力してしまうと、予測側が古いYtestを参照し
        # 精度が大きく劣化する（デプロイ環境で再現）。
        script_dir = os.path.dirname(os.path.abspath(__file__))
