...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def _json_response(self, data, status_code=200):
        ...
        try:
            ...
            _log(f"Response sent: status={status_code}, data={response[:200]}...")
        except Exception as e:
            _log(f"Error sending JSON response: {e}")
            try:
        ...
        try:
            if self.path == '/run-data':
                ...
                try:
                    ...
                    _log(f"JSON parse error in /run-data: {e}")
                    self._json_response({
                        'status': 'error',
                ...
                # エラー判定を強化
                if out.get('status') == 'error' or out.get('returncode') not in (0, None):
                    self._json_response(out, status_code=500)
                else:
                ...
                try:
                    ...
                    _log(f"JSON parse error in /run-train: {e}")
                    self._json_response({
                        'status': 'error',
                        'message': 'Invalid JSON payload',
                        'error': str(e)
                ...
                # Validate model name
                if model not in script_map:
                    _log(f"Invalid model name: {model}")
                    self._json_response({
                        'status': 'error',
                        'message': f'Invalid model name: {model}',
                        'valid_models': list(script_map.keys())
                    }, status_code=400)
                    return
                ...
                if data_out.get('returncode') not in (0, None):
                    self._json_response(data_out, status_code=500)
                    return
                ...
                result = {
                    'status': 'ok' if train_out.get('returncode') == 0 else 'error',
                    'stdout': combined_stdout,
                    'stderr': combined_stderr,
                    'returncode': train_out.get('returncode')
                }
                self._json_response(result, status_code=200 if result['status'] == 'ok' else 500)
                ...
                try:
                    ...
                    _log(f"Error in /run-tomorrow-data: {e}")
                    self._json_response({
                        'status': 'error',
                        'message': 'Failed to run tomorrow data scripts',
                        'error': str(e)
                    }, status_code=500)
                ...
                try:
                    ...
                    _log(f"JSON parse error in /run-tomorrow: {e}")
                    self._json_response({
                        'status': 'error',
                        'message': 'Invalid JSON payload',
                        'error': str(e)
    ...
    def do_GET(self):
        ...
        try:
            ...
            if self.path == '/available-years':
                ...
                try:
                    ...
                    self._json_response({
                        'status': 'ok',
                        'years': common,
                        'power_files_count': len(power_files),
                        'temp_files_count': len(temp_files)
                    })
                    return
                except Exception as e:
                    _log(f"Error in /available-years: {e}")
                    self._json_response({
                        'status': 'error',
            ...
            _log(f"Unexpected error in do_GET: {e}")
            self._json_response({
                'status': 'error',
                'message': 'Internal server error',
                'error': str(e)
