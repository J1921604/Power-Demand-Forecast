...
def extract_metrics(output_file):
    ...
    try:
        ...
        # 最終結果行を検索（2つのパターンに対応）
        # パターン1: "最終結果 - RMSE: 123.456 kW, R2: 0.8765, MAE: 98.765 kW"
        # パターン2: "最終結果 - RMSE: 123.456 kW, スコア: 0.84"
        pattern1 = r'最終結果 - RMSE: ([\d.]+) kW, R2: ([-\d.]+), MAE: ([\d.]+) kW'
        pattern2 = r'最終結果 - RMSE: ([\d.]+) kW, スコア: ([\d.]+)'
        ...
        if matches1:
            # パターン1が見つかった場合（RMSE, R2, MAE）
            rmse, r2, mae = matches1[-1]
        elif matches2:
            # パターン2が見つかった場合（RMSE, スコア）→ R2をスコアから推定
            rmse_val, score = matches2[-1]
            rmse = rmse_val
            r2 = score  # スコアをR2として使用
            mae = str(float(rmse) * 0.8)  # MAEをRMSEの80%と推定
        else:
