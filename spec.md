...
1. **Given** 2022,2023,2024年の電力需要データと気温データが存在する, **When** LightGBMモデルで訓練を実行する, **Then** RMSE≤500kW、R²≥0.80、MAE≤400kWを満たす学習済みモデルが生成される
2. **Given** 学習年を変更する, **When** 環境変数AI_TARGET_YEARSを設定する, **Then** 指定した年のデータのみで訓練が実行される
3. **Given** モデル訓練が完了する, **When** 訓練結果を確認する, **Then** RMSE/R²/MAEの精度指標が表示され、予測グラフ（PNG）が生成される
...
1. **Given** Open-Meteo APIが利用可能である, **When** 翌日予測用気温データを取得する, **Then** 過去7日+未来7日の気温データ（168時間分）が取得される
2. **Given** 学習済みモデルが存在する, **When** 翌日予測を実行する, **Then** 翌日24時間分の電力需要予測値（kW）が生成される
...
- **精度閾値違反**: R²<0.80を検出した場合、GitHub Issueを自動作成し、学習データの確認・ハイパーパラメータ再調整・異常値チェックを推奨する
...
- **予測結果（*_tomorrow.csv）**: 翌日24時間分の電力需要予測値（DATE, TIME, KW列）
...
flowchart TB
    ...
    B1 & B2 --> C1
    B3 --> C2
    C1 -->|X.csv/Y.csv| D1 & D2 & D3 & D4
    C2 -->|Xtest.csv| E1 & E2 & E3 & E4
    D1 & D2 & D3 & D4 -->|学習済みモデル| E1 & E2 & E3 & E4
