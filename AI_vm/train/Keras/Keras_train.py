...
class KerasConfig:
    ...
    DEFAULT_PATIENCE: int = 5  # オリジナル版と同じ設定
    NEURAL_NETWORK_UNITS: int = 64
    RANDOM_STATE: int = 42
...
def train_model_with_validation(model: Sequential,
    ...
    # Early Stoppingコールバック（オリジナル版完全再現）
    early_stopping = EarlyStopping(
        monitor='val_loss',
        min_delta=0.0,  # オリジナル版と同じ
        patience=5,  # オリジナル版と同じ
        restore_best_weights=True,
        verbose=0  # オリジナル版はverbose設定なし
    )
...
def save_learning_history_plot(history, history_png: str) -> None:
    ...
    plt.title('Keras 学習履歴', fontsize=12, pad=20)
    plt.xlabel('エポック', fontsize=12)
    plt.ylabel('損失 [kW^2]', fontsize=12)
    
    # 学習履歴プロット（日本語対応）
    epochs = range(1, len(history.history['loss']) + 1)
    plt.plot(epochs, history.history['loss'], 'b-', label='学習損失', linewidth=1.5)
    plt.plot(epochs, history.history['val_loss'], 'r-', label='検証損失', linewidth=1.5)
...
def evaluate_model_performance(model: Sequential,
    ...
    print(f'テストRMSE: {rmse:.2f}')
    print(f'テストR2スコア: {r2:.3f}')
...
def create_prediction_plots(y_pred: np.ndarray,
    ...
    try:
        ...
        plt.title('Keras Power Prediction (Full)', fontsize=12)
        ...
        plt.xlabel('Time', fontsize=12)
        plt.ylabel('Power [kW]', fontsize=12)
        plt.title('Keras Power Prediction (1 Week)', fontsize=12)
        plt.legend(fontsize=11)
        plt.xticks(fontsize=10)
        plt.yticks(fontsize=10)
        plt.tight_layout()
...
def main() -> None:
    ...
    try:
        ...
        # ファイルパス設定
        xtrain_csv = r"C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\data\Xtrain.csv"
        xtest_csv = r"C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\data\Xtest.csv"
        ytrain_csv = r"C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\data\Ytrain.csv"
        ytest_csv = r"C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\data\Ytest.csv"
        model_sav = r'C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\train\Keras\Keras_model.h5'
        ypred_csv = r'C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\train\Keras\Keras_Ypred.csv'
        ypred_png = r'C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\train\Keras\Keras_Ypred.png'
        ypred_7d_png = r'C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\train\Keras\Keras_Ypred_7d.png'
        history_png = r'C:\Users\Z9995695\Documents\UiPath\Python連携AI電力需要予測モデル_v2\AI\train\Keras\Keras_history.png'
