...
def predict_with_model(model, Xtomorrow_scaled: pd.DataFrame, y_test: pd.DataFrame) -> Tuple[pd.DataFrame, float, float, float]:
    ...
    return Ytomorrow_full, REG_RMSE, REG_SCORE, accuracy
...
def execute_tomorrow_prediction(config: LightGBMTomorrowConfig) -> Tuple[Optional[float], Optional[float]]:
    """統一されたLightGBM翌日予測処理"""
    ...
    if result is None:
        return None, None
    Ytomorrow, REG_RMSE, REG_SCORE, accuracy = result
