...
def create_temperature_dataframe(api_data: Dict[str, Any]) -> pd.DataFrame:
    ...
    'time'カラムも保持して期間調整に使用可能にする
    
    Args:
        api_data: Open-Meteo APIレスポンスデータ
        
    Returns:
        pd.DataFrame: 時系列特徴量付き気温データ（'time'カラム含む）
    ...
    try:
        ...
        # 'time'カラムも保持する（期間調整で使用）
        # 不要なオブジェクトを即座に削除は後で行う
        
        monitor_memory_usage("DataFrame作成後")
        logger.info(f"気温データフレーム作成完了: {len(df)}行, カラム: {list(df.columns)}")
        return df
...
def save_temperature_csv(df: pd.DataFrame, output_path: str, period_info_path: Optional[str] = None) -> None:
    """
    気温データをCSVファイルに保存（I/O最適化版）
    period_info.jsonがある場合は、その期間に合わせてデータを調整する
    
    Args:
        df: 保存対象データフレーム
        output_path: 出力CSVファイルパス
        period_info_path: 期間情報JSONファイルのパス（オプション）
    ...
    try:
        ...
        # デバッグ: period_info_pathの確認
        if period_info_path:
            logger.info(f"period_info_path指定あり: {period_info_path}")
            logger.info(f"period_info.json存在確認: {os.path.exists(period_info_path)}")
        else:
            logger.info("period_info_path指定なし")

        # period_info.jsonがある場合は、データを調整
        if period_info_path and os.path.exists(period_info_path):
            import json

            logger.info(f"period_info.json読み込み: {period_info_path}")
            with open(period_info_path, 'r', encoding='utf-8') as f:
                period_info = json.load(f)

            # Ytest.csvの期間情報を取得
            start_dt = pd.to_datetime(period_info['start_datetime'])
            end_dt = pd.to_datetime(period_info['end_datetime'])
            forecast_days = int(period_info['forecast_days'])
            past_hours = int(period_info['target_hours'])

            logger.info(f"期間情報読み込み: {start_dt} ～ {end_dt} ({past_hours}時間)")

            # dfに'time'カラムがあることを確認
            if 'time' in df.columns:
                # 過去7日分: Ytest.csvの期間に合わせる
                past_mask = (df['time'] >= start_dt) & (df['time'] <= end_dt)
                past_data = df[past_mask].copy()

                # 未来7日分: end_dt の次の時刻から forecast_days 日分
                ...
                df = pd.concat([past_data, future_data], ignore_index=True)
                logger.info(
                    f"期間調整完了: 過去{len(past_data)}行 + 未来{len(future_data)}行 = {len(df)}行"
                )
            else:
                logger.warning("'time'カラムが見つかりません。期間調整をスキップします。")

        # 学習・予測で想定している特徴量の列順に統一
        # 重要: *_tomorrow.py は CSV をそのまま numpy 変換して scaler に渡すため、列順ズレは致命的。
        ...
        # 入力データ不正（必須カラム不足など）は呼び出し側で扱えるようにそのまま送出
        raise
    except Exception as e:
...
def temp(
    ...
    try:
        ...
        # period_info.json のパスを構築（修正）
        # NOTE:
        #   - tomorrow/data.py は `AI/tomorrow/period_info.json` を生成する
        #   - 予測系（*_tomorrow.py）は cwd=AI を前提に `tomorrow/period_info.json` を参照する
        # ため、ここも `AI/tomorrow/period_info.json` を参照する。
        script_dir = os.path.dirname(os.path.abspath(__file__))
        period_info_path = os.path.join(script_dir, "period_info.json")

        # 旧仕様（誤出力）の残骸がある場合のフォールバック。
        # 通常は `AI/tomorrow/period_info.json` が存在するはず。
        legacy_period_info_path = os.path.join(script_dir, "tomorrow", "period_info.json")
        if (not os.path.exists(period_info_path)) and os.path.exists(legacy_period_info_path):
            logger.warning(
                "period_info.json が期待パスに見つからないため旧パスにフォールバックします: %s",
                legacy_period_info_path,
            )
            period_info_path = legacy_period_info_path
        
        # CSV保存（period_info_pathを渡す）
        save_temperature_csv(temperature_df, Xtomorrow_csv, period_info_path)
