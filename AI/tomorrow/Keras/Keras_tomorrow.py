...
def calculate_evaluation_metrics(ypred: np.ndarray, ytest: np.ndarray) -> Tuple[float, float]:
    ...
    try:
        ...
        if ypred.shape != ytest.shape:
            print(f"[INFO] ypred{ypred.shape}とytest{ytest.shape}の形状が異なります")
            
            # ytestの行数（過去7日分）でypredを調整
            test_length = ytest.shape[0]
            if ypred.shape[0] > test_length:
                # ypredの最後の7日分のみを精度評価に使用（最新データとの比較）
                ypred_for_eval = ypred[-test_length:]
                print(f"[OK] 精度評価用にypredを調整: {ypred_for_eval.shape} (最後{test_length}行使用)")
                ypred = ypred_for_eval
            elif ypred.shape[0] < test_length:
                # ypredが短い場合はytestを調整
                ytest = ytest[-ypred.shape[0]:]
                print(f"[OK] ytestを調整: {ytest.shape} (最後{ypred.shape[0]}行使用)")
