...
# スライス定数（dataclass の外で定義）
POWER_FILE_YEAR_SLICE = slice(5, 9)  # "juyo-YYYY.csv"のYYYY部分
TEMP_FILE_YEAR_SLICE = slice(12, 16)  # "temperature-YYYY.csv"のYYYY部分

# 統一設定インスタンス
config = DataConfig()

# CSVバリデーション関数
def validate_csv_file(file_path: str, required_columns: List[str], skiprows: int, encoding: str) -> bool:
    """
    CSVファイルの存在確認とカラム検証
    ...
    if file_size == 0:
        raise ValueError(f"CSVファイルが空です: {file_path}")
    
    logger.info(f"CSV検証中: {file_path} ({file_size:,} bytes)")
    ...
    try:
        ...
        # カラム数確認
        if len(df_sample.columns) < len(required_columns):
            raise ValueError(
                f"カラム数不足: 期待={len(required_columns)}, 実際={len(df_sample.columns)}"
            )
        
        logger.info(f"✓ CSV検証成功: {os.path.basename(file_path)} ({len(df_sample.columns)}カラム)")
        return True
        
    except pd.errors.ParserError as e:
        raise ValueError(f"CSVパースエラー: {file_path} - {e}")
    except UnicodeDecodeError as e:
        raise ValueError(f"エンコーディングエラー: {file_path} - {e}")
...
def load_data_files() -> Tuple[List[str], List[str]]:
    ...
    try:
        ...
        if env_years:
            ...
            if allowed:
                power_files = [f for f in power_files if os.path.basename(f)[POWER_FILE_YEAR_SLICE] in allowed]
                temp_files = [f for f in temp_files if os.path.basename(f)[TEMP_FILE_YEAR_SLICE] in allowed]
...
def get_common_years(power_files: List[str], temp_files: List[str]) -> Tuple[List[str], Dict[str, str], Dict[str, str]]:
    ...
    try:
        # ファイル名から年を抽出
        power_years = [os.path.basename(f)[POWER_FILE_YEAR_SLICE] for f in power_files]
        temp_years = [os.path.basename(f)[TEMP_FILE_YEAR_SLICE] for f in temp_files]
...
def load_power_data(power_files: List[str], power_map: Dict[str, str], common_years: List[str]) -> pd.DataFrame:
    ...
    for year in common_years:
        ...
        # float32型変換（メモリ最適化）
        if 'KW' in df.columns:
            df['KW'] = df['KW'].astype('int32')  # KWは整数値のためint32
...
def load_temperature_data(temp_files: List[str], temp_map: Dict[str, str], common_years: List[str]) -> pd.DataFrame:
    ...
    for year in common_years:
        ...
        validate_csv_file(
            temp_file,
            required_columns=["DATE", "TIME", "TEMP"],
